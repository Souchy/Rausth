use rausth::{auth::{JwtService, GitHubOAuthService}, config::Config};
use vercel_runtime::{run, Body, Error, Request, Response, StatusCode};
use serde_json::json;

/// Handle GitHub OAuth authorization request.
///
/// Generates an authorization URL and redirects the user to GitHub's login page.
#[tokio::main]
...
pub async fn handler(_req: Request) -> Result<Response<Body>, Error> {
    ...
    let oauth_config = match &config.auth.github {
        Some(cfg) if cfg.enabled => cfg,
        _ => {
            return Ok(Response::builder()
                .status(StatusCode::BAD_REQUEST)
                .body(Body::from(json!({"error": "GitHub OAuth is not configured or disabled"}).to_string()))?);
        }
    };
